...
use super::{chapter_resolver::ChapterTreeResolver, textbook::TextbookExtractor, ResourceExtractor};
use crate::{
    client::RobustClient,
    config::AppConfig,
    constants,
...
impl CourseExtractor {
    ...
    fn get_teacher_map(&self, data: &CourseDetailsResponse) -> HashMap<usize, String> {
        let teacher_id_map: HashMap<_, _> = data
            .teacher_list.as_deref().unwrap_or_default()
            .iter().map(|t| (t.id.as_str(), t.name.as_str())).collect();
        ...
        let streams = ti_items?
            ...
            .filter_map(|item| {
                ...
                let height_str = item
                    ...
                    .requirements.as_deref().unwrap_or_default()
                    .iter()
