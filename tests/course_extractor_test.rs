...
use sed_dl::{
    ...
    error::AppResult,
    extractor::{course::CourseExtractor, ResourceExtractor},
    DownloadJobContext,
};
use std::fs;
...
async fn test_course_extractor_parses_mock_response() -> AppResult<()> {
    ...
    // 从文件中读取模拟的API响应
    let mock_body = fs::read_to_string("tests/fixtures/course_response.json")
        .expect("无法读取模拟响应文件");

    // 定义当我们的代码请求特定URL时，模拟服务器应该如何回应
    let resource_id = "fake-course-id";
    let mock_endpoint = server
        .mock("GET", format!("/zxx/ndrv2/resources/{}.json", resource_id).as_str())
        .with_status(200)
    ...
    let args = Arc::new(Cli::parse_from([
        "sed-dl",                  // 程序名
        "--url", "unused_url",     // 满足 mode group 的要求
        "--video-quality", "best", // 这是测试需要的关键参数
        "--select", "all",         // 确保是非交互模式行为
    ]));
    ...
    let extractor_template = context.config.url_templates.get("COURSE_QUALITY").unwrap().clone();
    let extractor = CourseExtractor::new(context.http_client.clone(), context.config.clone(), extractor_template);
    ...
    assert!(video_path.contains("第一节课视频 - 课堂录像 [720p] - [张老师].ts"), "视频文件名格式不正确");
    ...
    assert!(pdf_path.contains("相关文档 - 教学课件 - [张老师].pdf"), "PDF文件名格式不正确");
    assert_eq!(pdf_info.ti_size, Some(102400), "PDF大小解析错误");
    assert_eq!(pdf_info.ti_md5, Some("d41d8cd98f00b204e9800998ecf8427e".to_string()), "PDF MD5解析错误");
